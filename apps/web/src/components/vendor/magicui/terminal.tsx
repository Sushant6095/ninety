...
// Vendor: magicui Terminal (https://magicui.design/docs/components/terminal) — re-skinned to Ninety tokens.
// Re-skin: `motion/react` → framer-motion; stock zinc/red/yellow/green chrome → bg-surface + three hairline
// discs; text-sm tracking-tight → font-mono text-caption; <pre> → wrapping div (booth lines are sentences);
// AnimatedSpan reveal runs at m.fast; prefers-reduced-motion → instant, no travel (typing included).
...
export const AnimatedSpan = ({
  ...
  return (
    <motion.div
      ...
      animate={shouldAnimate ? { opacity: 1, y: 0 } : { opacity: 0, y: reduce ? 0 : -4 }}
      transition={
        reduce
          ? m.reduced
          : { duration: m.fast / 1000, delay: sequence ? 0 : delay / 1000, ease: m.easeOut }
      }
      className={cn("grid font-mono text-caption", className)}
      onAnimationComplete={() => {
        if (!sequence) return;
...
export const TypingAnimation = ({
  children,
  className,
  duration = 25, // ms per character — a system log types fast, never theatre
  delay = 0,
  as: Component = "span",
  startOnView = true,
  ...props
  ...
  useEffect(() => {
    ...
    if (started) {
      // prefers-reduced-motion: the full line lands instantly — no character churn.
      if (reduce) {
...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "z-0 w-full overflow-hidden rounded-lg border border-hairline/60 bg-surface",
        className
      )}
    >
      {/* window chrome — three hairline discs, no traffic lights on a trading surface */}
      <div className="flex flex-row items-center gap-x-1.5 border-b border-hairline/60 px-3 py-2">
        <div className="h-2 w-2 rounded-full bg-hairline" />
        <div className="h-2 w-2 rounded-full bg-hairline" />
        <div className="h-2 w-2 rounded-full bg-hairline" />
      </div>
      {/* wrapping log body (not <pre> — booth lines are sentences and must wrap) */}
      <div className="p-3">
        <div className="grid gap-y-1.5 font-mono text-caption text-hi/90">{wrappedChildren}</div>
      </div>
    </div>
  );
