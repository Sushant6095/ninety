...
export function CreditPill({ credits }: CreditPillProps) {
  return (
    <Link
      ...
      className="group inline-flex items-center gap-2 rounded-chip bg-surface px-3 py-1 ring-1 ring-inset ring-hairline transition-colors duration-200 hover:ring-up/40 active:bg-hairline/40"
    >
      <span className="h-1.5 w-1.5 rounded-full bg-up shadow-[0_0_6px_var(--up)]" />
      <span className="num text-body font-medium text-hi">{fmt(credits)}</span>
