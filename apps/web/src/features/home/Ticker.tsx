    ...
    <div className="border-b border-hairline bg-bg">
      <div className="flex items-stretch gap-0 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {TICKER.map((t, i) => (
          ...
          >
            {t.minute != null ? (
              <span className="num text-label font-semibold text-up">{t.minute}&#39;</span>
            ) : (
              <span className="num text-label text-lo">{t.time ?? "—"}</span>
            )}
            <span className="text-label font-medium text-hi/90">{t.code}</span>
            {t.score && <span className="num text-label text-hi">{t.score}</span>}
            <span className="num text-label text-lo">{t.lead}</span>
            <LivePrice value={prices[i]} className="text-label font-medium text-hi" />
          </Link>
        ))}
      </div>
    </div>
