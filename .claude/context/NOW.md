...
DONE(txline): TxLineClient auth flow (guest JWT→chain subscribe→sign→activate · both headers · cache+refresh · origin↔cluster guard); verify script prints authenticated WC26 snapshot (= pnpm test); samples in docs/txline-samples. See ADR-013. Day-0 left: confirm F1 path + txoracle subscribe IDL.
